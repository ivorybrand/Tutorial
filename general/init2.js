...
function checksameurl(url1, url2) {
	...
	var url1 = url1.substring(pos1).toLowerCase().replace(/#/g,'');
	var pos2 = url2.indexOf('/p/');
	var url2 = url2.substring(pos2).toLowerCase().replace(/#/g,'');
...
//Get blog posts (blog section)
function get_blog_posts(posts_on_blog) {
		...
		$.ajax({
			...
			success: function (json) {
				...
				$('.blog-section').append(
					...
					translate_sentence('Read more') + '</div></a></center><br><br>'
				);
...
function footer_three_columns() {
	...
	showpagelink(contactuspage, translate_sentence('Contact us'));
	document.write('<br/>');
	showpagelink(blogpage, translate_sentence("The IvoryBrand's blog"));
...
*******************************************************************************************************************************************************/
	...
	//Show index blog posts
	function show_index_blog_posts(posts_on_blog) {
		document.write("<center><h1><a href='" + blogpage + "'>" + translate_sentence("The IvoryBrand's blog") + "</a></h1></center>");
		get_blog_posts(posts_on_blog);
	}
...
*******************************************************************************************************************************************************/
	...
	//Show blog section page content
	function show_blog_page_content(posts_on_blog) {
		if (checksameurl(location.href, blogpage) == true || get_pagetype() === 'index') {
			general_ads_2();				// General ads 2
			get_blog_posts(posts_on_blog);	// Show blog posts
		}
	}
